...
class MainWindow(QWidget):
    ...
    def _trigger_alert_action(self, pair: str, alert: dict, price: float):
        ...
        if play_sound:
            alert_side = "buy" if any(c.get("operator", "") in ("above", "crosses_above", ">=", ">") for c in conds) else "sell"
            self._safe_notify("notify_signal", alert_side, pair, price)
